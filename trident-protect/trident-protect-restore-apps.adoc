...
NOTE: 当您使用 Kopia 作为数据移动器恢复备份时，您可以选择在 CR 中或使用 CLI 指定注释来控制 Kopia 使用的临时存储的行为。请参阅 https://kopia.io/docs/getting-started/["文档"^]有关您可以配置的选项的更多信息。使用 `tridentctl-protect create --help`有关使用Trident Protect CLI 指定注释的更多信息，请参阅命令。
